/** Tour — recorrido guiado paso a paso + catálogo visual.
 * Versión sin Framer Motion en GuidedPanel para garantizar que los clicks responden.
 */
...
const GUIDED_STEPS = [
  { icon: Sparkles, accent: 'violet', chapter: 'Bienvenida',
    title: 'Bienvenido a EliteCards',
    body: 'Plataforma TCG competitiva por temporadas. 220 jugadores, 1.176 matches y 58 eventos sembrados. Todo lo que veas está corriendo con datos reales.',
    cta: null },
  { icon: KeyRound, accent: 'violet', chapter: 'Acceso',
    title: 'Logueate como Admin',
    body: 'admin@elitecards.cl / admin123 para ver todo. Como jugador, cualquier alias minúscula @elitecards.cl con password player123 (shadowkaiser, pixelmage…).',
    cta: { to: '/login', label: 'Abrir Login' } },
  { icon: Compass, accent: 'amber', chapter: 'Identidad',
    title: 'Ruta del Campeón',
    body: 'Sistema de progreso. 30 niveles repartidos en 7 rangos: Iniciado → Aprendiz → Duelista → Retador → Elite → Maestro → Campeón. Cada temporada se reinicia.',
    cta: { to: '/ruta', label: 'Ver la Ruta' } },
  { icon: Trophy, accent: 'amber', chapter: 'Ranking',
    title: 'Ranking competitivo',
    body: '3 tabs: Rating Glicko-2, EXP de temporada, Win Rate. Filtro por juego (MTG / Pokémon / YGO / OP / UA / Digimon). Hall of Fame para campeones históricos.',
    cta: { to: '/ranking', label: 'Ver Ranking' } },
  { icon: Calendar, accent: 'violet', chapter: 'Eventos',
    title: 'Torneos suizos + Live',
    body: '58 eventos generados con pairings y standings reales. Bracket SVG con seeding desde standings. Reporte en 1 click, standings se reordenan animados.',
    cta: { to: '/events', label: 'Lista de Eventos' } },
  { icon: Film, accent: 'rose', chapter: 'Cinematic',
    title: "Champion's Vision",
    body: '5 escenas cinematográficas con shader de black hole + IA narradora. Ideal para pantalla grande durante una premiación.',
    cta: { to: '/players/16/vision', label: "Ver Vision" } },
  { icon: Star, accent: 'amber', chapter: 'Wow',
    title: 'Cosmos Hall of Fame',
    body: '220 estrellas 3D distribuidas en Fibonacci sphere. Click en cualquier estrella te lleva al perfil del jugador.',
    cta: { to: '/cosmos', label: 'Entrar al Cosmos' } },
  { icon: Radio, accent: 'cyan', chapter: 'Wow',
    title: 'Holographic War Room',
    body: 'Dashboard estilo Mass Effect con scanlines CRT y 6 paneles de stats live. Para monitor secundario en eventos físicos.',
    cta: { to: '/warroom', label: 'Entrar al War Room' } },
  { icon: Swords, accent: 'rose', chapter: 'Juego',
    title: 'Deck Duel Simulator',
    body: 'Dos decks enfrentándose con cartas levitando y barras de vida. Útil para warmup o decidir un build-off entre jugadores.',
    cta: { to: '/duel?a=1&b=2', label: 'Iniciar Duel' } },
  { icon: ArrowLeftRight, accent: 'amber', chapter: 'Juego',
    title: 'Trade Simulator',
    body: 'Mesa partida con drag-and-drop. Cada lado calcula su balance USD en vivo usando precios reales de Scryfall.',
    cta: { to: '/trade?me=1&peer=2', label: 'Abrir Trade' } },
  { icon: Layers, accent: 'violet', chapter: 'Construcción',
    title: 'Deck Builder Visual',
    body: 'Autocomplete con card art de Scryfall, mana curve en tiempo real e insight con IA Claude.',
    cta: { to: '/decks/1/builder', label: 'Construir un deck' } },
  { icon: Camera, accent: 'fuchsia', chapter: 'Mobile',
    title: 'AR Elite ID',
    body: 'Activá la cámara y tu Elite ID Card aparece superpuesto en 3D. Se puede capturar como PNG para redes. Mejor en móvil.',
    cta: { to: '/ar-id', label: 'Abrir AR' } },
  { icon: Music, accent: 'fuchsia', chapter: 'Mobile',
    title: 'Audio-Reactive Visualizer',
    body: 'Shader que reacciona al micrófono. Bass produce swirls, treble produce sparkles. Perfecto para correr en pantalla durante eventos.',
    cta: { to: '/visualizer', label: 'Abrir Visualizer' } },
  { icon: Trophy, accent: 'cyan', chapter: 'Admin',
    title: 'Panel de Administración',
    body: 'Configurás juegos, formatos, sets, banlist, eventos, productos, misiones, achievements, anuncios y encuestas.',
    cta: { to: '/admin', label: 'Ir a Admin' } },
  { icon: CheckCircle2, accent: 'emerald', chapter: 'Final',
    title: 'Listo. A jugar.',
    body: 'Cmd/Ctrl+K para Command Palette en cualquier página. Abajo está el catálogo completo. ¡A romper packs!',
...
const SECTIONS = [
  {
    title: 'Fuera de este planeta', emoji: '🛸',
...
const ACCENT = {
  violet:  { ring: 'ring-violet-500/30',  text: 'text-violet-300',  bg: 'from-violet-500/20 to-violet-700/10',    glow: 'shadow-violet-500/30' },
  amber:   { ring: 'ring-amber-500/30',   text: 'text-amber-300',   bg: 'from-amber-500/20 to-orange-700/10',     glow: 'shadow-amber-500/30' },
  cyan:    { ring: 'ring-cyan-500/30',    text: 'text-cyan-300',    bg: 'from-cyan-500/20 to-blue-700/10',        glow: 'shadow-cyan-500/30' },
  rose:    { ring: 'ring-rose-500/30',    text: 'text-rose-300',    bg: 'from-rose-500/20 to-fuchsia-700/10',     glow: 'shadow-rose-500/30' },
  emerald: { ring: 'ring-emerald-500/30', text: 'text-emerald-300', bg: 'from-emerald-500/20 to-cyan-700/10',     glow: 'shadow-emerald-500/30' },
  fuchsia: { ring: 'ring-fuchsia-500/30', text: 'text-fuchsia-300', bg: 'from-fuchsia-500/20 to-violet-700/10',   glow: 'shadow-fuchsia-500/30' },
};
...
export default function Tour() {
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if (e.key === 'ArrowRight') setStep((s) => (s < GUIDED_STEPS.length - 1 ? s + 1 : s));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-violet-950/30 text-white">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-6 pt-10 pb-6">
          ...
          <p className="text-slate-400 mt-4 max-w-2xl text-base sm:text-lg">
            ...
            <span className="text-white font-bold">362 ratings Glicko-2</span>.
          </p>
      ...
      <div className="max-w-6xl mx-auto px-6 pb-20">
        {mode === 'guided'
          ? <GuidedPanel step={step} setStep={setStep} />
          : <BrowsePanel />
        }
...
/* ─────────────────────────  GUIDED PANEL — sin Framer ───────────────────────── */

function GuidedPanel({ step, setStep }) {
  const s = GUIDED_STEPS[step] || GUIDED_STEPS[0];
  const a = ACCENT[s.accent] || ACCENT.violet;
  const Icon = s.icon;
    ...
    <div>
      ...
      <div className="mt-2 mb-6">
        ...
        <div className="h-1 rounded-full bg-white/[0.06] overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-violet-500 via-fuchsia-400 to-amber-300 transition-all duration-500"
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center justify-between gap-3">
          ...
          {GUIDED_STEPS.map((_, i) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all ${
                i === step ? 'w-6 bg-white'
                : i < step ? 'w-1.5 bg-white/60 hover:bg-white/80'
        ...
        >
          {isLast ? <>Reiniciar <Play size={14} /></> : <>Siguiente <ChevronRight size={16} /></>}
        </button>
      </div>
    </div>
  );
...
function BrowsePanel() {
  return (
    <div>
      {SECTIONS.map((sec) => (
        <section key={sec.title} className="mt-10 first:mt-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {sec.items.map((item) => {
              const a = ACCENT[item.accent] || ACCENT.violet;
              return (
                <Link
                  key={item.to}
                  to={item.to}
                ...
                >
                  ...
                  <p className={`text-[10px] font-mono mt-2 ${a.text} opacity-50 truncate`}>{item.to}</p>
                </Link>
              );
            })}
          </div>
